...
person_list = []
cat_path = "../../data/thumbnail-objects/categories_inv_index"
for category in os.listdir(cat_path):
    with open(cat_path+'/'+category) as jsonFile:
        jsonObject = json.load(jsonFile)
        jsonFile.close()
    if 'person' in jsonObject:
        person_list.extend(jsonObject['person'])

for id in person_list:
    frame_name = id+"_high.jpg"
    gender_list = []
    emotion_list = []
    frame_dict = "../../data/thumbnails-not-cropped/"+frame_name
    path = Path(frame_dict)
    if path.is_file():
